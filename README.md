<div align="center">

<img src="https://kommodo.ai/i/KOYVRnqO5CaVGhKY1LWF" width="100%" alt="Kshitanshi">

# KSHITANSHI

### currently building my way up

</div>

---

Curious about how things work and even more interested in building them.

I’m currently exploring data, analytics, technology, and different areas that catch my attention. Most of what I learn comes from working on projects, experimenting with ideas, and figuring things out along the way.

I’m still discovering where I want to take this, but every project is helping me learn something new and get a little better at what I do.

<br>

<div align="center">

<a href="YOUR_LINKEDIN_LINK">LinkedIn</a>
&nbsp;&nbsp;•&nbsp;&nbsp;
<a href="https://www.instagram.com/kshitanshi_priya/">Instagram</a>
&nbsp;&nbsp;•&nbsp;&nbsp;
<a href="https://github.com/Kshitanshi">GitHub</a>

</div>

---

<table>
<tr>

<td width="34%" valign="top">

## PLAYER PROFILE

**Role**  
Commerce Student & Curious Learner

**Location**  
India

**Focus**  
Learning & Building

**Current Level**  
In Progress

</td>

<td width="66%" valign="top">

## CURRENT MISSIONS

<img src="https://kommodo.ai/i/jPBXJMK75VTSxEemNjX2" width="100%" alt="Current Missions">

<br>

Currently working on:

- Learning SQL and Power BI
- Building real world data projects
- Strengthening my analytics skills
- Exploring new tools and technologies
- Growing my portfolio through projects and experimentation

</td>

</tr>
</table>

---

## LOADOUT

<div align="center">

<img src="https://kommodo.ai/i/2xkdYWaiv31mGlDsYTjD" width="72%" alt="Kshitanshi Loadout">

</div>

### CURRENTLY MASTERED

**Advanced Excel**

I've completed an advanced Excel learning path and have been applying it through dashboard and data analysis projects.

**Skills I've worked with:**

`Pivot Tables` · `VLOOKUP` · `IF` · `SUM` · `AVERAGE` · `MAX` · `MIN` · `Charts` · `Dashboards` · `Data Analysis` · `AI`

---

## COMPLETED MISSIONS

### ZOMATO SALES DASHBOARD

**Status:** Completed  
**Tool:** Microsoft Excel  
**Type:** Data Analysis · Dashboarding

A hands-on Excel dashboard project built to analyse sales data, identify patterns and turn raw information into useful business insights.

**[View Project →](YOUR_ZOMATO_REPOSITORY_LINK)**

---

## ROADMAP

### Advanced Excel → SQL → Power BI → Projects → Internships → Personal Website

| Stage | Status |
|---|---|
| Advanced Excel | Completed |
| SQL | In Progress |
| Power BI | Next |
| More Projects | Next |
| Internships | Future |
| Personal Website | Coming Soon |

---

## CONNECT

<div align="center">

<a href="YOUR_LINKEDIN_LINK">LinkedIn</a>
&nbsp;&nbsp;•&nbsp;&nbsp;
<a href="https://www.instagram.com/kshitanshi_priya/">Instagram</a>
&nbsp;&nbsp;•&nbsp;&nbsp;
<a href="https://github.com/Kshitanshi">GitHub</a>

<br><br>

**Personal website — coming soon**

</div>
